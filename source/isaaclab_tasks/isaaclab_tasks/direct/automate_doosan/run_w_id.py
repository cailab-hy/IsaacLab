...
def update_task_param(task_cfg, assembly_id, if_train, if_log_eval, use_xy_align):
    # Read the file lines.
    ...
    for line in lines:
        ...
        elif "use_xy_align: bool =" in line:
            line = use_xy_align_pattern.sub(rf"\1{str(use_xy_align)}", line)  
...
def main():
    ...
    parser.add_argument(
        ...
        default="source/isaaclab_tasks/isaaclab_tasks/direct/automate_doosan/assembly_tasks_cfg.py",
    )
    ...
    # - CAI LAB ---------------------------------------------------------------------------------------------------
    parser.add_argument("--max_iterations", type=int, default=500, help="Number of iteration for policy learning.") # parser.add_argument("--max_iterations", type=int, default=1500, help="Number of iteration for policy learning.")
    parser.add_argument("--use_xy_align", action="store_true", help="Use key-point based reward separated into xy alignment and z alignment components.")
    # -------------------------------------------------------------------------------------------------------------
    args = parser.parse_args()

    update_task_param(args.cfg_path, args.assembly_id, args.train, args.log_eval, args.use_xy_align)
    ...
    if args.train:
        bash_command += " scripts/reinforcement_learning/rl_games/train.py --task=Isaac-AutoMate-Assembly-Doosan-Direct-v0"
        ...
        bash_command += " scripts/reinforcement_learning/rl_games/play.py --task=Isaac-AutoMate-Assembly-Doosan-Direct-v0"
